...
export default {
  name: "BnovoReportRevenueGraphAdr",
  components: {
    PlanTooltipContent,
    GraphTemplate,
    TooltipTemplate,
  ...
  lineStrategy: strategiesDictionary.notFilled,
  barStrategy: strategiesDictionary.default,
  data() {
    return { title: this.$t("ADR") };
  },
  computed: {
    ...mapGetters("revenueReport", ["categorySelectedText", "categorySelectedAttributeText"]),
    ...mapState("revenueReport", ["adrData", "externalFilters", "adrPlan"]),
    getChartDataSource() {
      return this.adrData;
    },
    getPlanDataSource() {
  ...
  created() {
    ...
    chartOptions.scales.yAxisKey.title.text = `${this.$t("ADR")}, ${this.hotel.currency_sign}`;
  ...
  methods: {
    getMainDatasetLabel({
      ...
      if (isSomePast && !isSomeFuture && !isTodayInRange) {
        result = this.$t("Итог ADR");
      } else if (!isSomePast && isSomeFuture) {
        result = this.$t("ADR на будущие даты");
      }

      return result;
    },
    getFutureDatasetLabel() {
      return this.$t("ADR на будущие даты");
    },
  },
};
</script>
