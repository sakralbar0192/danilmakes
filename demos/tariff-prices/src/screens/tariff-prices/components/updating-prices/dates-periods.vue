    ...
    <div :class="$style['dates-container']">
      ...
      >
        <b-related-dropdown-date-range
          ...
          :data-test="`tariff-mass-prices-date-period-${periodIndex}`"
...
export default {
  name: "BnovoTariffPricesAndRestrictionsUpdatingPricesDatesPeriods",
  ...
  watch: {
    datesPeriods: {
      handler(datesPeriods) {
        // Проверку на пересечение периодов нужно выполнять, когда периодов больше одного и во всех из них указаны дата начала и дата конца
  ...
  methods: {
    ...
    createDatePeriod() {
      ...
      } else this.$emit("need-scroll-to-alerts");
    },
      ...
      do {
        ...
        for (let i = 0; i < periods.length; i++) {
          ...
          for (let j = i + 1; j < periods.length; j++) {
            ...
            const currentStart = moment(current.period[0], PriceAndRestrictionsService.massiveUpdatingPricesDatesFormats.value);
            const currentEnd = moment(current.period[1], PriceAndRestrictionsService.massiveUpdatingPricesDatesFormats.value);
            const otherStart = moment(other.period[0], PriceAndRestrictionsService.massiveUpdatingPricesDatesFormats.value);
            const otherEnd = moment(other.period[1], PriceAndRestrictionsService.massiveUpdatingPricesDatesFormats.value);

            if (currentStart.isSameOrBefore(otherEnd) && currentEnd.isSameOrAfter(otherStart)) {
              current.period = [
                moment.min(currentStart, otherStart).format(PriceAndRestrictionsService.massiveUpdatingPricesDatesFormats.value),
                moment.max(currentEnd, otherEnd).format(PriceAndRestrictionsService.massiveUpdatingPricesDatesFormats.value),
              ];
              processed.add(j);
              changed = true;
            }
